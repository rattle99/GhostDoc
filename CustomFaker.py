...
def BITCOINADDRESS():
    bitcoin_address = fake.sha256(raw_output=False)[
        :34
    ]  # Generate a random SHA256 hash and truncate to 34 characters
...
def ETHEREUMADDRESS():
    ethereum_address = (
        "0x" + fake.sha256(raw_output=False)[:40]
    )  # Generate a random SHA256 hash and use first 40 characters
...
def LITECOINADDRESS():
    Litecoin_address = fake.sha256(raw_output=False)[
        :26
    ]  # Generate a random SHA256 hash and truncate to 34 characters
...
def CRYPTO():
    presidio_bitcoin_address = fake.sha256(raw_output=False)[
        :34
    ]  # Generate a random SHA256 hash and truncate to 34 characters
...
def PL_PESEL():
    ...
    checksum = (
        sum(digit * weight for digit, weight in zip(first_10_digits, weights)) % 10
    )
    if checksum != 0:
